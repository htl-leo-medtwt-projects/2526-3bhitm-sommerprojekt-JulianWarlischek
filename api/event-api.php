...
if (isset($_GET['inviteFriend']) && isset($_GET['eventToInvite'])) {
    ...
    $stmt->bind_param("ii", $_GET['inviteFriend'], $_GET['eventToInvite']);
    $stmt->execute();
...
if (isset($_GET['removeInvitedFriend']) && isset($_GET['eventToRemoveFrom'])) {
    $stmt = $conn->prepare("DELETE FROM User_Event WHERE userid = ? AND event_id = ?");
    $stmt->bind_param("ii", $_GET['removeInvitedFriend'], $_GET['eventToRemoveFrom']);
    $stmt->execute();
...
if (isset($_POST['update-event'])) {
    $_eventId = $conn->real_escape_string($_POST['event-id']);
    $_name = $conn->real_escape_string($_POST['name']);
    $_description = $conn->real_escape_string($_POST['description']);
    $_startDate = $conn->real_escape_string($_POST['startDate']);
    $_endDate = $conn->real_escape_string($_POST['endDate']);
    ...
    if (empty($_SESSION['errors'])) {
        $stmt = $conn->prepare("UPDATE Event SET name = ?, startDate = ?, endDate = ?, describtion = ?, dresscode_desc = ?, ranking = ?, location_id = ? WHERE event_id = ?");
        $stmt->bind_param("ssssssii", $_name, $_startDate, $_endDate, $_description, $_dressCode, $_ranking, $_location, $_eventId);
        $stmt->execute();
        $event_id = $conn->insert_id;

        $stmt = $conn->prepare("DELETE FROM Event_Snack WHERE event_id = ?");
        $stmt->bind_param("i", $_eventId);
        $stmt->execute();

        for ($i = 0; $i < count($_snacks); $i++) {
            $stmt = $conn->prepare("INSERT INTO Event_Snack (event_id, snack_id, count) VALUES (?, ?, ?)");
            $stmt->bind_param("iii", $_eventId, $_snacks[$i]['snack_id'], $_snacks[$i]['count']);
            $stmt->execute();
        }

        $stmt = $conn->prepare("DELETE FROM Event_Drink WHERE event_id = ?");
        $stmt->bind_param("i", $_eventId);
        $stmt->execute();

        for ($i = 0; $i < count($_drinks); $i++) {
            $stmt = $conn->prepare("INSERT INTO Event_Drink (event_id, drink_id, count) VALUES (?, ?, ?)");
            $stmt->bind_param("iii", $_eventId, $_drinks[$i]['drink_id'], $_drinks[$i]['count']);
            $stmt->execute();
        }

        $stmt = $conn->prepare("DELETE FROM Event_Game WHERE event_id = ?");
        $stmt->bind_param("i", $_eventId);
        $stmt->execute();

        for ($i = 0; $i < count($_games); $i++) {
            $stmt = $conn->prepare("INSERT INTO Event_Game (event_id, game_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $_eventId, $_games[$i]);
            $stmt->execute();
        }

        $stmt = $conn->prepare("DELETE FROM User_Event WHERE event_id = ?");
        $stmt->bind_param("i", $_eventId);
        $stmt->execute();

        for ($i = 0; $i < count($_sharedWith); $i++) {
            $stmt = $conn->prepare("INSERT INTO User_Event (userid, event_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $_sharedWith[$i], $_eventId);
            $stmt->execute();
        }
        ...
        if (empty($_FILES["event-cover-image"]["name"])) {
            ...
            if ($checkSum == 1 && move_uploaded_file($_FILES["event-cover-image"]["tmp_name"], $target_file)) {
                try {
                    ...
                    $stmt = $conn->prepare("UPDATE Event SET cover_image = ? WHERE Event_ID = ?");
                    $stmt->bind_param("ii", $imageId, $_eventId);
                    $stmt->execute();
                } catch (Exception $e) {
                    $_SESSION['errors'][] = "Error uploading file: " . $e->getMessage();
                }
        ...
        if (empty($_FILES['event-images']['name'][0])) {
            ...
            $stmt = $conn->prepare("DELETE FROM Event_Image WHERE event_id = ?");
            $stmt->bind_param("i", $_eventId);
            $stmt->execute();
            ...
            for ($i = 0; $i < count($_FILES['event-images']['name']); $i++) {
                ...
                if ($checkSum == 1 && move_uploaded_file($_FILES["event-images"]["tmp_name"][$i], $target_file)) {
                    try {
                        ...
                        $stmt = $conn->prepare("INSERT INTO Event_Image (event_id, image_id) VALUES (?, ?)");
                        $stmt->bind_param("ii", $_eventId, $imageId);
                        $stmt->execute();
                    } catch (Exception $e) {
                        $_SESSION['errors'][] = "Error uploading file: " . $e->getMessage();
                    }
        ...
        header("Location: ../project/pages/events.php?updateEvent=" . $event_id);
        exit();
    }
}
